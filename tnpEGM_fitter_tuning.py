...
parser.add_argument('--altBkgCheby', action='store_true'  , help = 'switch to Chebychev as alternate background model.')
    ...
    for ib in range(len(tnpBins['bins'])):
        if (args.binNumber >= 0 and ib == args.binNumber) or args.binNumber < 0:
            if args.altSig and not args.addGaus:
                if args.mcSig and ib in tnpConf.tnpParAltSigFitMCTuneByBin.keys():
                    tnpRoot.histFitterAltSig(  sampleToFit, tnpBins['bins'][ib], tnpConf.tnpParAltSigFitMCTuneByBin[ib] )
                elif (not args.mcSig) and ib in tnpConf.tnpParAltSigFitTuneByBin.keys():
                    tnpRoot.histFitterAltSig(  sampleToFit, tnpBins['bins'][ib], tnpConf.tnpParAltSigFitTuneByBin[ib] )
                else:
            ...
            elif args.altBkg and not args.altBkgCheby:
                if ib in tnpConf.tnpParAltBkgFitTuneByBin.keys():
                    tnpRoot.histFitterAltBkg(  sampleToFit, tnpBins['bins'][ib], tnpConf.tnpParAltBkgFitTuneByBin[ib] )
                else:
                    tnpRoot.histFitterAltBkg(  sampleToFit, tnpBins['bins'][ib], tnpConf.tnpParAltBkgFit )
	    elif args.altBkg and args.altBkgCheby:
                tnpRoot.histFitterAltBkg(  sampleToFit, tnpBins['bins'][ib], tnpConf.tnpParAltBkgFit_Chebychev, 1)
            else:
                if ib in tnpConf.tnpParNomFitTuneByBin.keys():
                    print ('use tuned parameters: ', tnpConf.tnpParNomFitTuneByBin[ib])
                    tnpRoot.histFitterNominal( sampleToFit, tnpBins['bins'][ib], tnpConf.tnpParNomFitTuneByBin[ib])
                else:
